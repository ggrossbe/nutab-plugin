...
jQuery(document).ready(function($){
	...
	$("div.srsPlan").each(function() {
		...
		if (!p.verein && !+p.alle && !+p.aktuell) {
			p.alle = "1";
			//msg = msg + "<p>Keinen Verein angegeben</p>";
			//o.html(msg);
			//return;
		}
		if (p.title) m = "<p>"+p.title+"</p>"; else m = "";
		/* msg = msg + m+"<p class=\"srsLaden\">" + (window.srsPlanMsg || "Spielplan " + (p.klasse || p.liganummer || "") + " wird geladen") + "</p>";
		msg = msg + m+"<p class=\"srsLaden\">" + "URL " + p.url + " wird geladen" + "</p>";
		msg = msg + "<p> Aufruf von " + basis + "fetch_table.php</p>";
		o.html(msg);
		*/
		p.spielplan = 1;
		jQuery.ajax({
			...
			beforeSend: function ( ) {
				//msg = msg + "<p>" + account_fetch_table.loading_message + ": " + JSON.stringify(p) + "</p>";
				//o.html(msg);
			},
			success: function ( data, textStatus) {
				//o.append("<p>Successfully loaded data: " + textStatus + "</p>");
	...
	var plan_format = [
		{ // alles fuer plan
		"header": "Tag;Datum;Zeit;Heim;Gast;Halle;Ergebnis",
		"fields": "Wochentag;Spieldatum/^..(..).(..).(..).*/$3.$2.$1;Spieldatum/.*T(..:..).*/$1;HeimTeam_Name_kurz;GastTeam_Name_kurz;Halle_Name_kurz;Heim+%:+Gast@sbb",
		"classCol": "l;c;c;l;l;l;c"
		},
		...
		"classCol": "c;c;l;l;l;l;c"
		},
		{ // ohne halle fuer plan
		"header": "Tag;Datum;Zeit;Heim;Gast;Ergebnis",
		"fields": "Wochentag;Spieldatum/^..(..).(..).(..).*/$3.$2.$1;Spieldatum/.*T(..:..).*/$1;HeimTeam_Name_kurz;GastTeam_Name_kurz;Heim+%:+Gast@sbb",
		"classCol": "l;c;c;l;l;c"
		},
	...
	$("div.srsPlanVerein").each(function() {
		...
		p.club = clubs.shift();
		if (p.title) m = "<p>"+p.title+"</p>"; else m = "";
		msg = msg + m+"<p class=\"srsLaden\">" + (window.srsPlanMsg || "Gesamt-Spielplan " + "" + " wird geladen") + "</p>";
		o.html(msg);
		p.spielplanverein = 1;
		var processPart = function() {
			...
			$.ajax({
			    ...
			    error: function(XMLHttpRequest, textStatus, errorThrown) {
				    msg = msg + "<p>Error in GET srsPlanVerein: " + textStatus + "</p>";
					o.html(msg);
			    }
			});
		}
