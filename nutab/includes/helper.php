...
function init($s) {
	...
	$rows = $x[0];
	if (preg_match(';Ort.*Nr\..*>Heim;ismU', $rows[0])) $keineSpNummer = 0; else $keineSpNummer = 1;
...
function init($s) {
	...
	foreach($rows as $row) {
		...
		$zeit = "00:00";
		if ($x[2]) $zeit = substr(trim($x[2]),0,5);
		if (!$tennis && $pokal and !x[7]) continue; // bei Pokal nehmen wir nur Spiele mit Gast (rest freilos)
		if (preg_match(';u;ismU', $x[2])) $zeit = "00:00";
		if (preg_match(';kampflos;ismU', $row)) $zeit = "00:01";
		preg_match(';(\d\d)\.(\d\d)\.(\d\d\d\d);ismU', $datum, $xx);
		$da = "{$xx[3]}.{$xx[2]}.{$xx[1]}";
