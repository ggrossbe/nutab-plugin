...
Liest Tabellen und Spielpl�ne aus nuLiga aus, und bringt sie in einem JSON Format zur�ck
Zu verwenden in tabellen.js


Parameter
url: zeigt direkt auf eine NuLiga URL f�r eine aktuelle Tabelle oder Spielplan
...
aktuell gibt es in nuLiga nicht mehr, ist aus ISS3 �briggeblieben, entfernen
...
Aufruf erfolgt �ber JSONP oder XHR und CORS
...
// damit preg_match_all auch f�r lange strings geht
 ...
 * damit nicht jeder Seiten-Refresh zu einem Roundtrip bis zum nuLiga Server f�hrt
 */
...
// h�lt die Verbindung mit nuliga, kann Seiten abrufen, und POSTs absenden
// umgestellt auf requests for php nehmen https://requests.ryanmccue.info/
// GG require_once("Requests-1.7.0/library/Requests.php");
// GG Requests::register_autoloader();
class NuLiga {
	var $cookies;
	function init($state) {
	...
	function get($url, $referer = "") {
		...
		$o = array(
			...
			"verify" => false, // true w�re besser, aber uns ist es egal mit welchem Server wir reden
			"timeout" => 2*60,
		...
		*/
		...
		/*$this->cookies = wp_remote_retrieve_cookies($r);
		$this->ref = $url;
		$ret = rutf(wp_remote_retrieve_body($r));
		*/
		if ($DEB) {
			$t1 = microtime(true); $t1s = date("H:i:s", (int)$t1); $t1m = $t1*1000%1000;
...
// url zu nuliga kann direkt angegeben sein
$team = "";
if ($url) {
	$url = str_replace("http:", "https:", $url);
	if (preg_match(';^https://(.*?)\.liga\.nu/.*?/nuLiga(.*?)\.woa.*?groupPage\?(.*);i', $url, $x)) {
		// Tabelle/Spielplan f�r dieses Gruppe
		$verband = $x[1];
		$sportart = $x[2];
		$qs = $x[3]."&";
		...
		// Tabelle/Spielplan f�r dieses Team
		$verband = $x[1];
		$sportart = $x[2];
		$qs = $x[3]."&";
...
if (!$verband) $verband = "oehb-handball";
if (!$sportart) $sportart = "HBAT";
if ($spielplanverein) {
	$spielplan = 0;
	$club = (int) $club;
	if ($alle) {
...
if ($u && ($gruppe || $club || $nurteam)) {
	...
	if ($spielplan and ($von or $bis)) {
		// von und bis als Datum zur�ck, in JS filtern wir dann
		$s = $e = "";
...
if (!$u) $r = "<error>Keine g�ltige URL angegeben</error>";

// nach JSON Konvertieren und zur�ckliefern

//error_reporting(E_ALL & ~(E_NOTICE|E_WARNING));
if (!$jn) {
	$r = "<wrapper>$r</wrapper>"; // topmost node geht verloren, f�r $jn pfad �ndern wir das nicht, wegen kompatibel
}
try {
